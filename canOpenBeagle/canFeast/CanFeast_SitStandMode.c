...
void sitStand(int state)
{
    ...
    // Physical homing to one side of array positions (sit or stand)
    ...
    // Initialize robot in reached home state
    // Send out preop->start commands w. SDO
    //Initialise 4 joints
    ...
    //sitstate goes from 0 to N, indicating the N+1 indices of the sitstandArrays
    ...
    while (1)
    {
        ...
        // Move forward in sitdown direction
        else if (button1Status == 1 && movestate == STATEIMMOBILE && sitstate < (arrSize - 1))
        {
        ...
        // Change to work to array size
        else if (sitstate < arrSize-1 && movestate == STATESITTING)
        {
        ...
        //Button has to be pressed & Exo not moving & array not at end. If true, execute move.
        else if (button2Status == 1 && movestate == STATEIMMOBILE && sitstate > 0)
        {
        ...
        else if (sitstate > 0 && movestate == STATESTANDING)
        {
            if (checkPos(&socket, sitStandArrayHip[sitstate - 1], sitStandArrayKnee[sitstate - 1]) == 1)
            {
                printf("Position reached.\n");
