    ...
    while (reset != CO_RESET_APP && reset != CO_RESET_QUIT && CO_endProgram == 0)
    {
        ...
        if (firstRun)
        {
            ...
            /*  use default attributes (NULL)
                Executes the start_routine, in this case rt_thread, with args NULL
                 if it returns it calls pthread_exit
                using the return balue as the exit status.
                It returns 0 if creation is succesfull and an error code if not.
            */
        ...
        reset = CO_RESET_NOT;
        printf("HERE WE GO WORK WORK WORK WORK!");
        int n = 0;
