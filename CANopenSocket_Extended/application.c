...
void app_programAsync(uint16_t timer1msDiff){
    ...
    char data [50];
    itoa(CO_OD_RAM.actualMotorPositions.motor1, data, 10);
    // This lines writes the info in "data" to the file pointer specified
    fputs(data, fp);
    itoa(CO_OD_RAM.actualMotorPositions.motor2, data, 10);
    // This lines writes the info in "data" to the file pointer specified
    fputs(data, fp);
    itoa(CO_OD_RAM.actualMotorPositions.motor3, data, 10);
    // This lines writes the info in "data" to the file pointer specified
    fputs(data, fp);
    itoa(CO_OD_RAM.actualMotorPositions.motor4, data, 10);
    // This lines writes the info in "data" to the file pointer specified
    fputs(data, fp);
    fputs("\n", fp);
    ...
    fclose(fp);
    // while(n<1000){
    //    //printf("%d\n", CO_OD_RAM.statusWords.motor1);
    //    tfgyhj = CO_OD_RAM.statusWords.motor1;
    //     n++;
    // }
